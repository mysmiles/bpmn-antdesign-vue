...
<template>
  <section>
    ...
    <a-modal v-model="visible" :width="800" title="部门选择" @ok="handleOk" @cancel="selectedRowKeys = participants;">
      <a-table
        :columns="column"
...
export default {
  ...
  methods: {
    ...
    handleOk() {
      ...
      const JsonValue = {};
      this.$set(JsonValue, 'departmentIds', this.participants);
      ...
      handler(val) {
        try {
          ...
          if (data.departmentIds) {
            this.valueArray = data.departmentIds;
          } else {
            this.valueArray = [];
          }
        } catch (e) {
