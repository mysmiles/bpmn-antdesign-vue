<template>
  <div class="panel-tab__content">
    ...
    <div class="element-drawer__button">
      <a-button type="primary" icon="plus" block @click="openListenerForm(null)">添加监听器</a-button>
    </div>
    ...
    <a-drawer :visible="listenerFormModelVisible" title="执行监听器" :width="`${width}px`" destroy-on-close closable @close="listenerFormModelVisible = false">
      <a-form-model size="small" :model="listenerForm" :label-col="labelCol" :wrapper-col="wrapperCol" ref="listenerFormRef" @submit.native.prevent>
        <a-form-model-item label="事件类型" prop="event" :rules="[{ validator: checkEventType, trigger: ['blur', 'change'] }, { required: true, message: '请选择事件类型', trigger: ['blur', 'change'] }]">
          ...
          <a-select v-else v-model="listenerForm.event">
        ...
        <template v-if="listenerForm.listenerType === 'scriptListener'">
          ...
          >
            <el-input v-model="listenerForm.scriptFormat" allow-clear />
          </a-form-model-item> -->
          <!-- <a-form-model-item
            label="脚本类型"
            prop="scriptType"
          ...
          >
            <el-input v-model="listenerForm.value" allow-clear />
          </a-form-model-item>
          <a-form-model-item
          ...
          >
            <el-input v-model="listenerForm.resource" allow-clear />
          </a-form-model-item> -->
    ...
    return {
      ...
      elementListenersColumns: [
        ...
        },
        {
          ...
          showOverflowTooltip: true,
          minWidth: '100px',
          customRender: (text) => { return listenerType[text] },
